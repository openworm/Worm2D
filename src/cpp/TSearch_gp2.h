...
template<class T>
void BinaryWriteVector(const vector<T> & vec, ofstream & bofs)
{
TVector<T> tvec;
tvec.SetBounds(1,vec.size());
for (int i=1;i<=tvec.Size();i++) tvec(i)=vec[i-1];
tvec.BinaryWriteVector(bofs);
}

template<class T>
void BinaryReadVector(vector<T> & vec, ifstream & bifs)
{
TVector<T> tvec;
vec.resize(0);
tvec.BinaryReadVector(bifs);
for (int i=1;i<=tvec.Size();i++) vec.push_back(tvec(i));
}
