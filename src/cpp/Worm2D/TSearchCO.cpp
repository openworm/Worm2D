...
void TSearch::DoMicrobialSearch(int ResumeFlag)
{
	...
	while (!SearchTerminated())
	{
		...
		// Every generation counts as N tournaments, where N is the size of the population
		for (int i = 1; i <= Population.Size(); i++){
			//cout << i << endl;

			// choose two different individuals from the population (within their deme)
			indA = rs.UniformRandomInteger(1,Population.Size());
			indB = indA + rs.UniformRandomInteger(1,demesize);
			indB = indB>Population.Size()?indB-Population.Size():indB;

			// evaluate their fitness
			perfA = EvaluateVector(Population[indA], RandomStates[indA]);
			perfB = EvaluateVector(Population[indB], RandomStates[indB]);

			// compare them, chose winner and loser
			if (perfA>perfB){
				winner=indA;
				loser=indB;
				AccPerf+=perfA;
				if (perfA>MaxPerf) {MaxPerf = perfA; bestindex=indA;};
				if (perfB<MinPerf) MinPerf = perfB;
			}
			else {
				winner=indB;
				loser=indA;
				AccPerf+=perfB;
				if (perfB>MaxPerf) {MaxPerf = perfB; bestindex=indB;};
				if (perfA<MinPerf) MinPerf = perfA;
			}

			// replace loser with winner
			Population[loser]=Population[winner];

			// and mutate
			MutateVector(Population[loser]);

		}
		BestPerf = MaxPerf;
		AvgPerf = AccPerf/Population.Size();
